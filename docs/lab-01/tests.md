...
|---|---|---|---|---|
| API-01 | Supertest | Server (`/api/health`) | `/api/health` returns HTTP 200 and `{ status: "ok", service: "TokTickIT API" }` | Pending (Issue 2) |
...
| UI-03 | Vitest | Client (App) | Displays useful error message when API fails | Pending (Issue 4) |
